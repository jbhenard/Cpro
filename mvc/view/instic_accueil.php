...
if (!empty($_POST["submit_CEC"]) > 0) {    
    ...
    if (count($errsCEC) == 0) {        
        $parametres['CEC']['titre']='';        
        $parametres['CEC']['nom']=$_POST["nom"];
        $parametres['CEC']['prenom']=$_POST["prenom"];
        $parametres['CEC']['user']=$email;
        $parametres['CEC']['mdp']=$_POST["mdp1"];
        
        $_SESSION['tab']=$parametres;                           
        $_SESSION['user'] = $email;        

        header("Location: ../controller/registerCEC.php");
        die();
    }
}
...
//===============================================================================================
if (!empty($_POST["submit_login"]) > 0) {

    if (empty($_POST["mail_login"])) $errs_login["mail"][] = "Veuillez renseigner l'Adresse de courriel!";
    if (empty($_POST["mdp_login"])) $errs_login["mdp"][] = "Veuillez renseigner le mot de passe!";

    $user=$_POST['mail_login'];    

    if (!filter_var($user, FILTER_VALIDATE_EMAIL)) {
        //L'email n'est pas bonne.
        $errs_login["verifmmail"][] = "Veuillez renseigner une Adresse de courriel valide!";
    }
    ...
    //Selection pour récupérer le MdP
    try {
        $reponse = $bdd->query("SELECT * FROM personne WHERE user = '$user' ORDER BY clef DESC LIMIT 1;");
        ...
        if ($mdp == $mdpBDD) {
            ...
            $errs_login["MdPko"][] = "Veuillez renseigner un Mot de Passe correct!";
        }
    }
    catch(Exception $e) {        
        $errs_login["connexionko"][] = "Veuillez renseigner une Adresse de courriel existante!";
    }

    if (count($errs_login) == 0) {
        $_SESSION['user'] = $user;
        header("Location: instic_espacecandidat.php");
        die();
    }
}
            ...
            <div class="container">
                <section class="content-full-width" id="primary">
                    ...
                    <div class='dt-sc-hr '></div>
                        ...
                        <div  class='column dt-sc-one-third  '><h3 style="text-align:center;">Page d'accueil</h3></div>
                        ...
                        <div  class='column dt-sc-one-third  first'>
                            <!-- Code JB HENARD -->
                            <h4 class="blanc" id="h4menu">Vous souhaitez vous inscrire?</h4>
                            ...
                            <label for="edit-mail" class="blanc">Votre mail<span class="form-required blanc" title="Ce champ est obligatoire.">*</span></label>
                            ...
                            <input type="submit" id="edit-submit" name="submit_CEC" value="Créer un nouveau compte" class="form-submit" />
                            </div>
                            </form>
                        </div>
                        ...
                        <div  class='column dt-sc-one-third  first'>                       
                        <h4 class="blanc" id="h4menu">Vous avez déjà un compte?</h4>                                                                
                        <form method="post" id="user-login-form" accept-charset="UTF-8">
                        <div>
                        <div class="form-item form-type-textfield form-item-name">
                        <label for="edit-name" style="color:#ffffff;">Adresse de courriel<span class="form-required" title="Ce champ est obligatoire." style="color:#ffffff;">*</span></label>
                         <input type="text" id="mail_login" data-inputmask="'alias': 'email'" name="mail_login" value="<?php if(isset($_Post['mail_login'])) {echo $_POST['mail_login'];} ?>" size="15" maxlength="60" class="form-text required" />
                        </div>
                        <div class="form-item form-type-password form-item-pass">
                        <label for="edit-pass" style="color:#ffffff;">Mot de passe <span class="form-required" title="Ce champ est obligatoire." style="color:#ffffff;">*</span></label>
                        <input type="password" id="mdp_login" name="mdp_login" value="<?php if(isset($_Post['mdp_login'])) {echo $_POST['mdp_login'];} ?>" size="15" maxlength="60" class="form-text required" />                        
                        </div>  
                        <div class="form-actions form-wrapper" id="edit-actions">
                        <input type="submit" id="edit-submit" name="submit_login" value="Se connecter" class="form-submit" />
                        </div>                      
                        <br/><br/><br/>
                        <p><strong><a href="instic_nouveaumdp.php" title="Demander un nouveau mot de passe par courriel." style="color:#ffffff;">
                        Mot de passe oublié?</a></strong></p>
                        </div>
                        ...
                        if (count($errs_login) > 0) {
                            ...
                            foreach ($errs_login as $champEnErreur => $erreursDuChamp) {
