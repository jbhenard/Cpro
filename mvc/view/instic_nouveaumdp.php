...
if (!empty($_POST["submit"]) > 0) {
    ...
    if (empty($_POST["nom"]) && empty($_POST["mail"])) $errs["nom"][] = "Veuillez renseigner le Nom et/ou l'Adresse de courriel!";
    //if (empty($_POST["mail"])) $errs["nom"][] = "Veuillez renseigner le Nom et/ou l'Adresse de courriel!";
    ...
    if ($pos === false && !empty($_POST["nom"])) {
        //echo "La chaîne '$findme' ne se trouve pas dans la chaîne '$email'";
        //Si la saisie est un Nom
        try
        {
            // Insertion du message à l'aide d'une requête préparée
            $reponse = $bdd->query("SELECT * FROM personne WHERE nom = '$nom'") or  die('Erreur SELECT !');
            $row = $reponse->fetch(PDO::FETCH_ASSOC);
                $user = $row['user'];
                $id = $row['id'];

            //$reponse = $bdd->query("SELECT * FROM personne WHERE user = '$user';");
            //echo "<br/> Rt si Nom= ".$reponse->rowCount();

            while($row = $reponse->fetch(PDO::FETCH_ASSOC)) {
                echo '<br/>'.utf8_decode($row['nom']).' '.utf8_decode($row['user']).' '.utf8_decode($row['mdp'])."<br/>";  //etc...
                $user = $row['user'];
                $id = $row['id'];
                echo '<br/> user = '.$user;
            }
        ...
        catch(Exception $e)
        {
            ...
            $errs["nomko"][] = "Veuillez renseigner le Nom existant!";
        }
    } else {
        //echo "La chaine '$findme' a été trouvée dans la chaîne '$email'";
        //echo " et débute à la position $pos";

        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            //L'email n'est pas bonne.
            $errs["verifmmail"][] = "Veuillez renseigner une Adresse de courriel valide!";
        }
        else {
            ...
            {
                // Insertion du message à l'aide d'une requête préparée
                $reponse = $bdd->query("SELECT * FROM personne WHERE user = '$email'") or  die('Erreur SELECT !');
                $row = $reponse->fetch(PDO::FETCH_ASSOC);
                $user = $row['user'];
                $id = $row['id'];
                //echo "<br/> Rt si mail= ".$reponse->rowCount();
            ...
            catch(Exception $e)
            {
                // En cas d'erreur, on affiche un message et on arrête tout
                //die('Erreur SELECT : '.$e->getMessage());
                $errs["emailko"][] = "Veuillez renseigner une Adresse de courriel existante!";
            }
            }
        }
    }
    ...
    if (count($errs) == 0) {
        ...
        $nouveaumdp='achanger';
        $reponse = $bdd->query("UPDATE personne SET mdp='$nouveaumdp' WHERE user ='$user';") or  die('Erreur SELECT !');

        //echo "<br/> Rt MàJ MdP= ".$reponse->rowCount();

        //Méthode $_GET ci-après
        echo '<br/> user= '.$user;
        echo '<br/> id= '.$id;
        header("Location: http://localhost/cpro_dev/instic_gmail.php?user=$user&id=$id&nouveaumdp=$nouveaumdp");
        die();
    }
}
?>
                ...
                <section class="content-full-width" id="primary">
                    ...
                    <span class="wds_slideshow_image_spun1_0">
                      <span class="wds_slideshow_image_spun2_0">
                        ...
                        <!-- Code JB HENARD -->
                            <h4 class="blanc" id="h4menu">Demander un nouveau mot de passe</h4>
                            <div id="tasks">
                            ...
                            <form method="post" id="user-register-form" accept-charset="UTF-8">
                                <div class="form-item form-type-textfield form-item-name">
                                    ...
                                    <input type="text" id="edit-name" name="nom" value="<?php if(isset($_Post['nom'])) {echo $_POST['nom'];} ?>" size="60" maxlength="254" class="form-text required" />
                                </div>
                                ...
                                <div class="form-item form-type-textfield form-item-mail">
                                    ...
                                    <div class="description">
                                        Saisir soit un nom, soit une adresse de courriel valide. Le système enverra tous les courriels à cette adresse électronique.
                                ...
                                <div class="form-actions form-wrapper" id="edit-actions">
                                    <input type="submit" id="edit-submit" name="submit" value="Demander un nouveau MdP par courriel" class="form-submit" />
                                </div>
                            </form>
